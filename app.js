/* API_BASE_URL can be set before this file loads, e.g. window.VoxaConfig={API_BASE_URL:'https://api.example.com'} */
...
['keydown'].forEach(event=>question.addEventListener(event,e=>{if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)){e.preventDefault();submitButton.click();}}));
...
const languages=['English','Hindi','Kannada'];let langIndex=0;setInterval(()=>{$('rotating-language').textContent=languages[++langIndex%languages.length]},2100);setupBars();renderHistory();